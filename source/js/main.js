...
const initPage = () => {
  ...
  safeRun("typed", () => {
    ...
    // The subtitle element only exists on home pages (rendered when
    // `is_home()`), so checking for it covers the root page as well as
    // localized home pages generated by hexo-generator-i18n (e.g. /en/,
    // /zh-TW/), where `location.pathname` never equals `config.root`.
    if (shouldInitTyped && document.getElementById("subtitle")) {
      initTyped("subtitle");
    }
  });
